...
@implementation GetFollowingServices
+(GetFollowingServices *)sharedInstance{
    static GetFollowingServices *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetFollowingServices alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)getfollowinPostswithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@%@%@",@"http://www.defindme.com/webservices/users/get_followings/",self.strfrndId,@"/",self.strmyId,@"/1/10/"];
