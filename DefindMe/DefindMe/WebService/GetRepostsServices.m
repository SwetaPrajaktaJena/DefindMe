...
@implementation GetRepostsServices
+(GetRepostsServices *)sharedInstance{
    static GetRepostsServices *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetRepostsServices alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)getRepostsswith:(NSString *)strpostID withPageNo:(NSString *)strpageNo withCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@",@"http://www.defindme.com/webservices/posts/getReposts"];
