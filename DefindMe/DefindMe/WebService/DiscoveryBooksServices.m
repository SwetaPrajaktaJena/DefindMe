...
@implementation DiscoveryBooksServices
+(DiscoveryBooksServices *)sharedInstance{
    static DiscoveryBooksServices *discoveryService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        discoveryService=[[DiscoveryBooksServices alloc] initWithService:LOG_IN];
    });
...
-(void)bookPostsDetailsServicewithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@%@",@"http://www.defindme.com/webservices/medias/getDiscovery/",[LoginService sharedInstance].uId,@"/3/1/",self.indexValue];
