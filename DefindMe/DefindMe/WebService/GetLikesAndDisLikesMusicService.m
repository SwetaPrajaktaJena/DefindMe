//
//  GetLikesAndDisLikesMusicService.m
//  DefindMe
//
...
@implementation GetLikesAndDisLikesMusicService
@synthesize musiclike;
@synthesize strSearch;
+(GetLikesAndDisLikesMusicService *)sharedInstance{
    static GetLikesAndDisLikesMusicService *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetLikesAndDisLikesMusicService alloc] initWithService:LOG_IN];
    });
    return bookService;
}
...
-(void)getLikePostswithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@%@",@"http://www.defindme.com/webservices/medias/getLikesIphone/",self.musiclike,@"/1/1/10/",strSearch];
