...
@implementation DiscoveryMoviesServices
+(DiscoveryMoviesServices *)sharedInstance{
    static DiscoveryMoviesServices *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[DiscoveryMoviesServices alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)moviePostsDetailsServicewithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@%@",@"http://www.defindme.com/webservices/medias/getDiscovery/",[LoginService sharedInstance].uId,@"/2/1/",self.indexValue];
