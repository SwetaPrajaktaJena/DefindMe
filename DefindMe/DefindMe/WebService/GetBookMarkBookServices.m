...
@implementation GetBookMarkBookServices
+(GetBookMarkBookServices *)sharedInstance{
    static GetBookMarkBookServices *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetBookMarkBookServices alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)bookmarkBookServicewithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"http://www.defindme.com/webservices/medias/getBookmarkAllIphone/4946/3/1/"];
