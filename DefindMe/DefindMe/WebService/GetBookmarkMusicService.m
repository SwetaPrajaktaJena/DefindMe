...
@implementation GetBookmarkMusicService
@synthesize strSearch;
+(GetBookmarkMusicService *)sharedInstance{
    static GetBookmarkMusicService *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetBookmarkMusicService alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)bookmarkMusicServicewithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"http://www.defindme.com/webservices/medias/getBookmarkAllIphone/4946/1/1/"];
