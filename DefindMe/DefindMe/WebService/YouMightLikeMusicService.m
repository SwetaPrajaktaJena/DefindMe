...
@implementation YouMightLikeMusicService
+(YouMightLikeMusicService *)sharedInstance{
    static YouMightLikeMusicService *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[YouMightLikeMusicService alloc] initWithService:LOG_IN];
    });
    return bookService;
}
...
-(void)getMightLikePostswithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@",@"http://www.defindme.com/webservices/medias/getMightLikeMedia/",[LoginService sharedInstance].uId,@"/1"];
