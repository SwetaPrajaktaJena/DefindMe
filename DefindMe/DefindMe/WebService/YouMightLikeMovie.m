...
@implementation YouMightLikeMovie
+(YouMightLikeMovie *)sharedInstance{
    static YouMightLikeMovie *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[YouMightLikeMovie alloc] initWithService:LOG_IN];
    });
    return bookService;
}
...
-(void)getMightLikePostswithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@",@"http://www.defindme.com/webservices/medias/getMightLikeMedia/",[LoginService sharedInstance].uId,@"/2"];
