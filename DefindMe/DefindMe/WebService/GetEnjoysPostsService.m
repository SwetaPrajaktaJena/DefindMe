...
@implementation GetEnjoysPostsService
+(GetEnjoysPostsService *)sharedInstance{
    static GetEnjoysPostsService *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetEnjoysPostsService alloc] initWithService:LOG_IN];
    });
    return bookService;
}
-(void)getenjoyswith:(NSString *)strpostID withUseID:(NSString *)strUID withPageNo:(NSString *)strpageNo withCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@",@"http://www.defindme.com/webservices/posts/getEnjoys"];
