//
//  GetLikesAndDisLikesMovieService.m
//  DefindMe
//
...
@implementation GetLikesAndDisLikesMovieService
@synthesize strSearch;
+(GetLikesAndDisLikesMovieService *)sharedInstance{
    static GetLikesAndDisLikesMovieService *bookService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bookService=[[GetLikesAndDisLikesMovieService alloc] initWithService:LOG_IN];
    });
    return bookService;
}
...
-(void)getLikePostswithCompletionHandler:(CompletionHandler)handler{
    ...
    NSString * url=[NSString stringWithFormat:@"%@%@%@%@",@"http://www.defindme.com/webservices/medias/getLikesIphone/",self.moviesLike,@"/2/1/10/",strSearch];
